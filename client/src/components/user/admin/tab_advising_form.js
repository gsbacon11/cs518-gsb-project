...
import {
  apiAdminGetSheets,
  apiAdminGetSheetDetails,
  apiAdminUpdateSheetStatus,
} from "@/app/api";
...
export default function TabAdvisingSheet() {
  ...
  const [headerInfo, setHeaderInfo] = useState(
    "View Sheets To See More Details",
  );
  ...
  const onView = async (e, staticRowIndex, table, row) => {
    const data1 = await apiAdminGetSheetDetails(
      cookies.get("api_token"),
      row.original.sheetID,
    );
    var tmp_pre = [];
    var tmp_course = [];
  ...
  const onAccept = async (e) => {
    ...
    await apiAdminUpdateSheetStatus(
      cookies.get("api_token"),
      selectedRow.sheetID,
      "Accepted",
      selectedRow.email,
    ...
    <main>
      <div className="pb-5">
        {isViewing == true ? (
          <span>
            <div className="flex flex-row place-content-evenly">
              <div className="border border-slate-300 h-40 w-60 rounded-xl">
                <label className="text-2xl pl-5 text-center font-bold text-white">
                  Viewing
                </label>
                <div className="border border-slate-300"></div>
                <p className="pl-1 text-white">
                  Sheet ID: {selectedRow.sheetID}
                </p>
                <p className="pl-1  text-white">
                  Date Submitted: {selectedRow.date}
                </p>
                <p className="pl-1  text-white">
              ...
              <div className="border border-slate-300 h-40 w-60 rounded-xl">
                ...
                <div className="border border-slate-300"></div>
                <p className="pl-1  text-white">
                  Last Term: {selectedRow.termLast}
                </p>
                <p className="pl-1  text-white">GPA: {selectedRow.gpa}</p>
                <p className="pl-1  text-white">
                  Current Term: {selectedRow.termCurrent}
                </p>
              </div>
              <div className="border border-slate-300 h-40 w-60 rounded-xl">
                <label className="text-2xl pl-5 text-center font-bold text-white">
                ...
                {selectedPrereqs.map((val, i) => (
                  <p key={i} className="pl-1  text-white">
                    {val}
                  </p>
                ))}
              </div>
              ...
              <span className="flex flex-col align-bottom place-content-evenly w-28 ">
                <button
                  className="bg-white  border-2 border-green-500 font-bold rounded-xl uppercase leading-normal"
                  onClick={(e) => onAccept(e)}
                >
                  Accept
                </button>
                <button
                  className="bg-white  border-2 border-red-500 font-bold rounded-xl uppercase leading-normal"
          ...
          <span>
            <label className="text-4xl pl-5 text-center font-bold text-white">
              {headerInfo}
            </label>
          </span>
        )}
