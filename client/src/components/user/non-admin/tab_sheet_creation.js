...
export default function SheetCreation() {
  ...
  const onEffect = async () => {
    ...
    const data4 = await apiGetCoursesTaken(
  ...
  function addPrereq() {
    ...
    setPrereqRows([
      ...preReqRows,
      <LevelCourseRow
        index={preReqRows.length}
        allOptions={allPrereqs}
        optionsLevels={prereqOptionsLevelLoaded}
        selection={selectedPrereqs}
        setSelection={setSelectedPrereq}
      />,
    ]);
  }
  ...
  function addCourse() {
    ...
    setCourseRows([
      ...courseRows,
      <LevelCourseRow
        index={courseRows.length}
        allOptions={allCourses}
        optionsLevels={courseOptionsLevelLoaded}
  ...
  const onSubmit = async (e) => {
    ...
    if (preReqRowNum != 0) {
      ...
      preReqRows[preReqRowNum - 1].props.selection.forEach(
        (selection, index) => {
          if (selection.Level == 0) {
            tmp_errors.push(
              "In Pre-requisites  (row " +
                index +
                "): Invalid (no level selection)",
            );
          }
          if (selection.Course == "") {
            tmp_errors.push(
    ...
    if (coursesNum != 0) {
      ...
      courseRows[coursesNum - 1].props.selection.forEach((selection, index) => {
        if (selection.Level == 0) {
          tmp_errors.push(
            "In Courses (row " + index + "): Invalid (no level selection)",
          );
        }
        if (selection.Course == "") {
          tmp_errors.push(
            "In Courses (row " + index + "): Invalid  (no course selection)",
          );
        }
      });
    }
  ...
  return (
    <div className={styles.mainFormUser}>
      ...
      <div className={styles.simpleDivision}></div>
      {preReqRows.map((val, i) => preReqRows[i])}
      ...
      {courseRows.map((val, i) => courseRows[i])}
        ...
        {infoState == 2 ? (
          <span>
            ...
            <p className="text-center">
              You will get an email when the admin has responded to your
              submission.
            </p>
          </span>
        ) : (
