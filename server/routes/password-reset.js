...
router.post("/",(req,res)=>{
    try {
        ...
        database.execute("update users set password=?, passwordReset=1 where email=?",
        ...
        ,function(err, result){
            if(result.affectedRows==0){
                ...
                sendEmail(req.body.email, "ODU Course Advising Portal Password Reset", "Temporary Password: " + temp_password);
            }
        })
    } catch(error){
...
router.post("/onlogin", verifyToken, (req,res)=>{
    try {
        ...
        database.execute("update users set password=?, passwordReset=0 where userID=?",
