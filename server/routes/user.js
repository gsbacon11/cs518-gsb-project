...
router.post("/admin/approve-users", verifyToken, (req,res) => {
    try {
        for(var i = 0; i < 1; ++i){
            ...
            sendEmail(req.body.emails[i], "ODU Course Advising Portal Account Approved", "Your account has offcially been approved by the administrator! Feel free to login at any time.");
        }
        res.status(200).send([]);
    } catch(error){
        console.log(error.message);
